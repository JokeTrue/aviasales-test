...
from settings import FASTEST, SLOWEST, CHEAPEST, EXPENSIVE, OPTIMAL, ROUTE_CACHE_KEY, INFO_SERVICE_API_URL, REDIS_HOST, \
    ALLOWED_MODES
...
async def get_flights(request, mode):
    ...
    if cached_data:
        ...
        cache.setex(cache_key, timedelta(hours=1), json.dumps(data, default=json_defaults, sort_keys=True))
...
async def get_optimal_flights(request):
    ...
    most_optimal = set(
        ...
        set(fastest_graph_routes[:len(cheapest_graph_routes) // 2]),
    )
